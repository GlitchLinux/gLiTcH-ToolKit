...
tar --lzma -xvf "$REPO_DIR/MULTIBOOTFILES.tar.lzma" --no-same-owner -C "$MOUNT_POINT"

# Extract mini-ventoy without preserving ownership
...
cp "$REPO_DIR/grubfmia32.efi" "$MOUNT_POINT/EFI/BOOT/"
cp "$REPO_DIR/grubfmaa64.efi" "$MOUNT_POINT/EFI/BOOT/"
cp "$REPO_DIR/grubfm.elf" "$MOUNT_POINT/boot/grub/"
cp "$REPO_DIR/grubfm_multiarch.iso" "$MOUNT_POINT/boot/grub/"
cp "$REPO_DIR/loadfm" "$MOUNT_POINT/boot/grub/"
cp "$REPO_DIR/fmldr" "$MOUNT_POINT/"
cp "$REPO_DIR/ventoy.dat" "$MOUNT_POINT/"
cp "$REPO_DIR/efi.img" "$MOUNT_POINT/"
