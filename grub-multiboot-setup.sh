...
verify_install() {
    ...
    # BIOS
    if [[ "$HAS_BIOS" == true ]]; then
        local check_dev="${bios_part:-$disk}"
        local embedded=""
        embedded=$(strings "$check_dev" 2>/dev/null | grep 'search.fs_uuid' | head -1) || true
        if [[ -n "$embedded" ]]; then
            ok "BIOS core.img: ${WHT}${embedded}${RST}"
        else
            embedded=$(dd if="$disk" bs=512 skip=1 count=62 2>/dev/null | strings | grep 'search.fs_uuid' | head -1) || true
            if [[ -n "$embedded" ]]; then
